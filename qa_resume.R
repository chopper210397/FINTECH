...
for (selected_agent in name_of_sheets) {
    assign(
             ...
             range_read("https://docs.google.com/spreadsheets/d/12VXX9J2UixC3IWMwmwzoGXgRCWl8KoONXS2t_bEqeEM/edit?usp=sharing",
                        sheet = selected_agent , col_types = "Dnccc")
          )
}

for ( x in gsub(" ","_",name_of_sheets)) {
  
  colnames(x)<-colnames("Nelson_Rubio")
  
  x %>% mutate(nombre= 
                  for (i in length(name_of_sheets) )  {
                    
                  print(name_of_sheets[i]) } )
}
