...
vinculado %>% filter(Coordinador!="NA")%>% filter(Perfil!="NA")%>% count(mes,Constante) %>% 
  ggplot(aes(x=mes,y=n,label=n))+geom_bar(stat = "identity",fill="lightblue")+geom_text()+
...
# zona por mes
vinculado%>%  filter(Coordinador!="NA")%>% filter(Perfil!="NA") %>%
  filter(Zona %in% unique(top20zonas$Zona)) %>% count(mes,Zona) %>%
